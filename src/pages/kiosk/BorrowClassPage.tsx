      ...
      {!isLoading && (
        <div className="flex-1 flex flex-col justify-center items-center my-auto w-full">
          ...
          {step === 'active-loan-warning' && student && activeLoan && (
            <div key="active-loan-warning" className="animate-kiosk-step flex flex-col items-center gap-6 max-w-xl mx-auto w-full">
              ...
              <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-xl shadow-slate-200/50 space-y-5">
                ...
                <div>
                  ...
                  {!selectedTeacher ? (
                    ...
                    /* ─── KASUS 3: Hanya 1 Mapel ─── */
                    <div className="w-full px-4 py-3 rounded-2xl border-2 border-blue-600 bg-blue-50/90 text-blue-950 flex items-center justify-between gap-3 shadow-xs">
                      <span className="text-xs sm:text-sm font-extrabold leading-snug line-clamp-2">
                        {selectedTeacher.subjects[0].nama}
                      </span>
                      <div className="w-5 h-5 rounded-full border-2 border-blue-600 bg-blue-600 text-white flex items-center justify-center flex-shrink-0">
            ...
            <div key="photo" className="animate-kiosk-step flex flex-col items-center justify-center gap-6 max-w-xl mx-auto w-full my-auto">
              ...
              <WebcamCapture
                onCapture={(b64) => {
                  setPhotoPreview(b64)
                  setStep('confirm')
                  uploadService.photo(b64, 'borrow')
                    .then(({ path }) => setPhotoPath(path))
                    .catch((err) => console.warn('Gagal upload foto kelas:', err))
                }}
              ...
              <div className="bg-white rounded-3xl p-4 sm:p-6 lg:p-8 w-full space-y-2 sm:space-y-3 border border-slate-200/80 shadow-xl shadow-slate-200/50">
                {photoPreview && (
                  <div className="flex items-center gap-3 sm:gap-4 p-2 sm:p-3 bg-slate-50 border border-slate-200/80 rounded-2xl mb-1 sm:mb-2">
                    ...
                    />
                    <div className="min-w-0">
                      <p className="text-[10px] sm:text-xs font-bold text-slate-500 uppercase tracking-wider">Perwakilan Peminjam</p>
