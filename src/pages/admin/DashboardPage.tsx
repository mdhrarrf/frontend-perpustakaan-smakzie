...
export function AdminDashboard() {
  ...
  const chart  = useQuery({ queryKey: ['dashboard-chart'],  queryFn: () => dashboardService.chart(7) })
  const today  = useQuery({ queryKey: ['loans-today'],      queryFn: loanService.today })
    ...
    <div className="space-y-6">
      ...
      {stats.isLoading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard label="Buku Hilang Pending" value={s.buku_hilang_pending} icon={<PackageX size={20} />} color="yellow" />
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {chart.isLoading ? (
              ...
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={chart.data.loan_chart} barSize={12}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Bar dataKey="pinjam" name="Dipinjam" fill="#6366f1" radius={[4,4,0,0]} />
                  <Bar dataKey="kembali" name="Dikembalikan" fill="#22c55e" radius={[4,4,0,0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </CardBody>
        ...
        <Card>
          ...
          <CardBody className="p-0">
            {chart.data?.top_books?.map((book, idx) => (
              <div key={book.id} className="flex items-center gap-3 px-6 py-3 border-b border-slate-50 last:border-0">
                <span className="text-xs font-bold text-slate-400 w-4">{idx + 1}</span>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-slate-800 truncate">{book.judul}</p>
                  <p className="text-xs text-slate-400">{book.penulis}</p>
                </div>
                <span className="text-sm font-semibold text-primary-600">{book.jumlah_dipinjam}x</span>
              </div>
            ))}
          </CardBody>
        </Card>
